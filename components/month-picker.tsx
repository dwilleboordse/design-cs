...
export function MonthPicker({
  ...
  const current = value ?? state.currentMonthId;
  const idx = ids.indexOf(current);
  const change = onChange ?? setCurrentMonth;

  return (
    <div className="inline-flex items-center rounded-md border border-border overflow-hidden">
      <button
        className="px-2 py-1.5 hover:bg-panel2 disabled:opacity-30"
        onClick={() => change(ids[Math.max(0, idx - 1)])}
        disabled={idx <= 0}
      ...
      <button
        className="px-2 py-1.5 hover:bg-panel2 disabled:opacity-30"
        onClick={() => change(ids[Math.min(ids.length - 1, idx + 1)])}
